    ...
    if fmass != 0.0:
        ...
        if pp == 0.0:
            ...
            if pp3 == 0.0:
                ...
                chi1 = complex(
                    nh * p[1] / sqrt(2.0 * pp * pp3), p[2] / sqrt(2.0 * pp * pp3)
                )
        ...
        if nh == 1:
            fi[2] = complex(0.0, 0.0)
            fi[3] = complex(0.0, 0.0)
            fi[4] = chi[0]
            fi[5] = chi[1]
        else:
            fi[2] = chi[1]
...
def oxxxxx(p, fmass, nhel, nsf):
    ...
    if fmass != 0.0:
        ...
        if nh == 1:
            fo[2] = chi[0]
            fo[3] = chi[1]
            fo[4] = complex(0.0, 0.0)
            fo[5] = complex(0.0, 0.0)
        else:
            fo[2] = complex(0.0, 0.0)
            fo[3] = complex(0.0, 0.0)
            fo[4] = chi[1]
            fo[5] = chi[0]
...
def vxxxxx(p, vmass, nhel, nsv):
    ...
    if vmass != 0.0:
        ...
        if pp == 0.0:
            vc[2] = complex(0.0, 0.0)
            vc[3] = complex(-nhel * sqh, 0.0)
            vc[4] = complex(0.0, nsvahl * sqh)
            vc[5] = complex(hel0, 0.0)

        else:
            emp = p[0] / (vmass * pp)
            vc[2] = complex(hel0 * pp / vmass, 0.0)
            ...
            if pt != 0.0:
                ...
                vc[3] = complex(-nhel * sqh, 0.0)
                vc[4] = complex(0.0, nsvahl * sign(sqh, p[3]))
    else:
        pp = p[0]
        pt = sqrt(p[1] ** 2 + p[2] ** 2)
        vc[2] = complex(0.0, 0.0)
        ...
        if pt != 0.0:
            ...
            vc[3] = complex(-nhel * sqh, 0.0)
            vc[4] = complex(0.0, nsv * sign(sqh, p[3]))
