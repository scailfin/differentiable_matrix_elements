...
class FlatInvertiblePhasespace(VirtualPhaseSpaceGenerator):
    ...
    @staticmethod
    def bisect(v, n, target=1.0e-16, maxLevel=80):
        ...
        while level < maxLevel:
            ...
            checkV = (u ** (n + 1)) * (n + 2.0 - (n + 1.0) * u)
            error = abs(1.0 - checkV / v)
            if error == 0.0 or error <= target:
                break
            left *= 2.0
            right *= 2.0
            if v <= checkV:
    ...
    def setInitialStateMomenta(self, output_momenta, E_cm):
        ...
        if self.n_initial == 1:
            ...
            if self.initial_masses[0] == 0.0 or self.initial_masses[1] == 0.0:
                output_momenta[0] = LorentzVector([E_cm / 2.0, 0.0, 0.0, +E_cm / 2.0])
                output_momenta[1] = LorentzVector([E_cm / 2.0, 0.0, 0.0, -E_cm / 2.0])
            else:
                ...
                output_momenta[0] = LorentzVector([E1 / 2.0, 0.0, 0.0, +Z / 2.0])
                output_momenta[1] = LorentzVector([E2 / 2.0, 0.0, 0.0, -Z / 2.0])
        return
    ...
    def get_PS_point(self, random_variables):
        ...
        if any(math.isnan(r) for r in random_variables):
            logger.warning(
                "Some input variables from the integrator are malformed: %s"
                % (
                    ", ".join(
                        "%s=%s" % (name, random_variables[pos])
                        for name, pos in self.dim_name_to_position.items()
                    )
                )
            )
            logger.warning(
    ...
    def generateKinematics(self, E_cm, random_variables):
        ...
        if self.n_final == 1:
            ...
            output_momenta.append(LorentzVector([mass / 2.0, 0.0, 0.0, +mass / 2.0]))
            output_momenta.append(LorentzVector([mass / 2.0, 0.0, 0.0, -mass / 2.0]))
    ...
    def invertIntermediatesMassless(self, K, E_cm, random_variables):
        ...
        for i in range(2, self.n_final):
            u = (K[i - 1] / K[i - 2]) ** 2
            random_variables[i - 2] = (self.n_final + 1 - i) * math.pow(
                u, self.n_final - i
